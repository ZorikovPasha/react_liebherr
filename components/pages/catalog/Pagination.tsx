...
const Pagination: React.FC<{ query: React.MutableRefObject<string> }> = ({
  query,
}) => {
  ...
  const betWeenLength = React.useRef(
    pages.length > 5 ? pages.length - 4 : pages.length === 5 ? 1 : 0,
  )
  ...
  const onPageChange = (num: number) =>
    dispatch(fetchProducts(handleQuery(num)))
    ...
    <div className="pagination flex aic">
      ...
      <ul className="pagination__list flex">
        {pages.slice(0, between.current?.left || 2).map((num) => (
          <li
            key={num}
            className={`pagination__item flex aic jcc ${
              currentChunk === num ? 'pagination__item--active' : ''
            }`}
        ...
        {pages
          .slice(
            (betWeenLength?.current &&
              between?.current &&
              between.current.right) ||
              pages.length - 3,
          )
          .map((num) => (
            <li
              key={num}
              className={`pagination__item aic flex jcc ${
                currentChunk === num ? 'pagination__item--active' : ''
              }`}
              onClick={onPageChange.bind(null, num)}
            >
