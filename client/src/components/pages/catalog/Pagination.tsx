...
const Pagination: React.FC<{ query: React.MutableRefObject<string> }> = ({ query }) => {
  ...
  const between = React.useRef(
    pages.length > 5 ? { left: 2, right: pages.length - 2 } : pages.length === 5 ? { left: 2, right: 4 } : null,
  )
  ...
  return (
    <div className="pagination flex aic">
      ...
      <ul className="pagination__list flex">
        ...
        {pages
          .slice((betWeenLength?.current && between?.current && between.current.right) || pages.length - 3)
          .map((num) => (
            <li
              key={num}
